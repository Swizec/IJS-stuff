...
#ifdef WIN32
        uint64_t tosend = min((uint64_t)1<<12,complete-req->offset);
#else
...
void HttpGwNewRequestCallback (SOCKET http_conn){
    ...
    req->xcontentdur = (char *)malloc(strlen(url)-42+1);
    strcpy(req->xcontentdur,&url[42]);
    dprintf("%s @%i demands %s %s %s\n",tintstr(),req->id,hash,req->headers[HTTPGW_RANGE],req->xcontentdur);
